# Manual de Usuario — Glyphix ⚕️

Bienvenido al manual oficial de **Glyphix**, la plataforma de Historia Clínica Electrónica (HCE) multi-tenant de grado médico para especialistas y clínicas modernas.
...
Glyphix ha sido desarrollado bajo tres pilares tecnológicos irrenunciables:
...
Glyphix está estructurado con una arquitectura multi-tenant (multi-clínica) y multi-doctor corporativa.

### Flujo de Acceso Inicial
1. **Registro Gratuito:** Al crear una cuenta en Glyphix, recibes un **Trial de 7 días sin tarjeta de crédito**.
...
Para ahorrar tiempo buscando expedientes, Glyphix incorpora una potente barra de comandos rápida.
...
El motor clínico de Glyphix organiza las consultas médicas a través de un flujo estructurado de 6 etapas que agilizan el diagnóstico.
...
La característica más robusta de Glyphix es su capacidad para operar desconectado de la red.
...
* Al perder la señal de internet, Glyphix entra en **Modo Local**. Puedes seguir operando sin bloqueos.
...
Cada consulta sellada en Glyphix genera una firma **hash criptográfica SHA-256 encadenada**. El hash del registro actual incorpora el contenido de la consulta y el hash de la consulta anterior. Esto asegura que nadie pueda modificar los registros pasados directamente en la base de datos sin romper la cadena, ofreciendo blindaje jurídico absoluto ante auditorías legales.
...
Para exprimir todo el potencial de Glyphix, te sugerimos seguir esta rutina en tu clínica:
