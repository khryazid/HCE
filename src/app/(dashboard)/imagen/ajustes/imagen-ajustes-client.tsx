...
export function ImagenAjustesClient() {
  const { tenant, loading } = useTenant();
  const router = useRouter();

  useEffect(() => {
    if (!loading && tenant) {
      // Solo imaging, clinic_admin, y owner pueden ver esto
      const allowedRoles = ["imaging", "clinic_admin", "owner"];
    ...
    <div className="flex flex-col gap-8 w-full hce-page max-w-5xl mx-auto lg:flex-row">
      ...
      <aside className="lg:w-64 shrink-0">
        <div className="sticky top-24">
          <div className="mb-6 px-3">
            <h1 className="text-2xl font-bold text-ink tracking-tight mb-1">Ajustes de Imagen</h1>
            <p className="text-sm text-ink-soft">Configura tu área de imagenología</p>
          </div>
      ...
      <main className="flex-1 min-w-0 pb-24">
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          <LabSettingsPanel department="imaging" />
        </div>
      </main>
    </div>
  );
